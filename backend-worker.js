...
async function apiResponse(request, env) {
  ...
  if (url.pathname === `${API_PREFIX}/solar/overview` && request.method === "GET") {
    ...
    const summary = await env.DB.prepare(`SELECT SUM(CASE WHEN transaction_status IN ('COMPLETED','PARTIAL') THEN 1 ELSE 0 END) AS completed_transactions,SUM(CASE WHEN transaction_status IN ('QR_CREATED','READY','DISPENSING') THEN 1 ELSE 0 END) AS pending_qr_count,SUM(CASE WHEN transaction_status='CANCELLED' THEN 1 ELSE 0 END) AS cancelled_qr_count,SUM(CASE WHEN transaction_status IN ('COMPLETED','PARTIAL') AND requested_liters>0 AND metered_liters IS NOT NULL AND ABS(metered_liters-requested_liters)/requested_liters>.02 THEN 1 ELSE 0 END) AS not_match_count,SUM(CASE WHEN transaction_status IN ('FAILED','MANUAL_REVIEW') THEN 1 ELSE 0 END) AS failed_review_count,COALESCE(SUM(CASE WHEN transaction_status IN ('COMPLETED','PARTIAL') THEN metered_liters ELSE 0 END),0) AS metered_liters,COALESCE(SUM(CASE WHEN transaction_status IN ('COMPLETED','PARTIAL') THEN requested_liters ELSE 0 END),0) AS requested_liters,COALESCE(AVG(CASE WHEN transaction_status IN ('COMPLETED','PARTIAL') AND requested_liters>0 THEN metered_liters/requested_liters*100 END),0) AS fulfillment_percent,SUM(CASE WHEN transaction_status='MANUAL_REVIEW' OR (transaction_status IN ('COMPLETED','PARTIAL') AND requested_liters>0 AND ABS(metered_liters-requested_liters)/requested_liters>.02) THEN 1 ELSE 0 END) AS review_count FROM solar_fueling_transaction WHERE COALESCE(fueling_completed_at,qr_created_at,fueling_started_at,source_updated_at,ingested_at)>=? AND COALESCE(fueling_completed_at,qr_created_at,fueling_started_at,source_updated_at,ingested_at)<?`).bind(from,to).first();
    ...
    const totalizer = await env.DB.prepare("SELECT (SELECT machine_totalizer_liters FROM solar_fueling_transaction WHERE transaction_status IN ('COMPLETED','PARTIAL') AND machine_totalizer_liters IS NOT NULL AND fueling_completed_at>=? AND fueling_completed_at<? ORDER BY fueling_completed_at,transaction_id LIMIT 1) AS first_totalizer_liters,(SELECT machine_totalizer_liters FROM solar_fueling_transaction WHERE transaction_status IN ('COMPLETED','PARTIAL') AND machine_totalizer_liters IS NOT NULL AND fueling_completed_at>=? AND fueling_completed_at<? ORDER BY fueling_completed_at DESC,transaction_id DESC LIMIT 1) AS latest_totalizer_liters").bind(from,to,from,to).first();
    ...
    const firstTotalizer=totalizer?.first_totalizer_liters==null?null:Number(totalizer.first_totalizer_liters), latestTotalizer=totalizer?.latest_totalizer_liters==null?null:Number(totalizer.latest_totalizer_liters), machineDelta=firstTotalizer==null||latestTotalizer==null?null:latestTotalizer-firstTotalizer, metered=Number(summary?.metered_liters||0);
    return json({ data_mode:"ACTUAL_DATABASE", range:{from,to}, summary:{...summary,system_stock_liters:Number(config.opening_stock_liters||0)+Number(movement.net||0)-Number(consumed.liters||0),first_totalizer_liters:firstTotalizer,latest_totalizer_liters:latestTotalizer,machine_delta_liters:machineDelta,totalizer_variance_liters:machineDelta==null?null:machineDelta-metered,metering_match_percent:machineDelta!=null&&machineDelta>0?Math.max(0,100-Math.abs(machineDelta-metered)/machineDelta*100):null,totalizer_reset_count:0,stock_accuracy_percent:latestOpname?.accuracy_percent??null,live_stock_liters:latestLevel?.stock_liters??null,live_level_percent:null,live_level_quality:latestLevel?(levelStale&&latestLevel.quality==='GOOD'?'STALE':latestLevel.quality):'NO_DATA',live_level_quality_reason:levelStale?'Tidak ada pembaruan sensor lebih dari 3 menit':latestLevel?.quality_reason??null,live_level_source_ts:latestLevel?.source_ts??null}, latest_opname:latestOpname||null,time_series:trend.results,level_time_series:levelTrend.results,user_ranking:users.results,exceptions:[] });
  }
  ...
  if (url.pathname === `${API_PREFIX}/solar/transactions` && request.method === "GET") {
    ...
    const filter="COALESCE(fueling_completed_at,qr_created_at,fueling_started_at,source_updated_at,ingested_at)>=? AND COALESCE(fueling_completed_at,qr_created_at,fueling_started_at,source_updated_at,ingested_at)<? AND (?='%%' OR qr_code LIKE ? OR COALESCE(requester_name,'') LIKE ? OR COALESCE(processed_by,'') LIKE ?) AND (?='ALL' OR transaction_status=?)";
    const args=[from,to,search,search,search,search,status,status];
    const rows=await env.DB.prepare(`SELECT * FROM solar_fueling_transaction WHERE ${filter} ORDER BY COALESCE(fueling_completed_at,qr_created_at,fueling_started_at,source_updated_at,ingested_at) DESC LIMIT ? OFFSET ?`).bind(...args,pageSize,offset).all();
