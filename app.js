...
function motorHistoricalLogPanel(motor, phaseHistory, rangeLabel) {
  ...
  return `<section class="motor-log-panel" aria-label="Motor historical data log">
    ...
    <div class="motor-log-table-wrap" tabindex="0"><table class="data-table motor-log-table"><thead><tr><th>Timestamp</th><th>Phase R</th><th>Phase S</th><th>Phase T</th><th>Avg. Current</th><th>Line Voltage</th><th>kW</th><th>Hz</th><th>Imbalance</th><th>Status</th></tr></thead><tbody>${rows.map((row) => `<tr><td class="mono" data-label="Timestamp">${formatDateTime(row.timestamp, true)}</td><td class="mono phase-cell" data-label="Phase R"><strong>${row.currentR.toFixed(1)} A</strong><small>${row.voltageR.toFixed(1)} V</small></td><td class="mono phase-cell" data-label="Phase S"><strong>${row.currentS.toFixed(1)} A</strong><small>${row.voltageS.toFixed(1)} V</small></td><td class="mono phase-cell" data-label="Phase T"><strong>${row.currentT.toFixed(1)} A</strong><small>${row.voltageT.toFixed(1)} V</small></td><td class="mono" data-label="Avg. Current"><strong>${row.currentAverage.toFixed(1)} A</strong></td><td class="mono" data-label="Line Voltage">${row.lineVoltage.toFixed(0)} V</td><td class="mono" data-label="kW">${row.kw.toFixed(1)}</td><td class="mono" data-label="Hz">${row.hz.toFixed(1)}</td><td class="mono" data-label="Imbalance">${row.imbalance.toFixed(1)}%</td><td data-label="Status"><span class="data-pill ${row.status === "Review" ? "warning" : "good"}">${row.status}</span></td></tr>`).join("")}</tbody></table></div>
...
function motorPhaseTroubleshootingPanel(motor, phaseHistory, rangeLabel) {
  ...
  return `<section class="motor-troubleshooting-panel" aria-label="Three phase troubleshooting analysis">
    ...
    <div class="motor-phase-table-wrap" tabindex="0"><table class="data-table motor-phase-table"><thead><tr><th>Phase</th><th>Current Min / Avg / Max</th><th>Time High</th><th>Time Low</th><th>Voltage Min / Avg / Max</th><th>Condition</th></tr></thead><tbody>${phaseHistory.phases.map((phase) => {
      ...
      return `<tr><td><span class="phase-name phase-${phase.phase.toLowerCase()}">${phase.phase}</span></td><td class="mono"><span class="compact-stat"><small>MIN</small>${phase.currentMin.toFixed(1)}<small>AVG</small>${phase.currentAverage.toFixed(1)}<small>MAX</small><strong>${phase.currentMax.toFixed(1)}</strong> A</span></td><td class="mono">${showTime(phase.highTime)}</td><td class="mono">${showTime(phase.lowTime)}</td><td class="mono"><span class="compact-stat"><small>MIN</small>${phase.voltageMin.toFixed(1)}<small>AVG</small>${phase.voltageAverage.toFixed(1)}<small>MAX</small>${phase.voltageMax.toFixed(1)} V</span></td><td><span class="data-pill ${phaseTone}">${phase.currentMax > limit ? "High review" : "Within limit"}</span></td></tr>`;
