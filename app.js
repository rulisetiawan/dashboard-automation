...
function motorHistoricalLogPanel(motor, phaseHistory, rangeLabel) {
  ...
  return `<section class="motor-log-panel" aria-label="Motor historical data log">
    ...
    <div class="motor-log-table-wrap" tabindex="0"><table class="data-table motor-log-table"><thead><tr><th>Timestamp</th><th>Phase R</th><th>Phase S</th><th>Phase T</th><th>Load</th><th>Drive</th><th>Quality</th></tr></thead><tbody>${rows.map((row) => `<tr><td class="mono" data-label="Timestamp">${formatDateTime(row.timestamp, true)}</td><td class="phase-cell" data-label="Phase R"><span><small>RMS Amp</small><strong>${row.currentR.toFixed(1)} A</strong></span><span><small>Voltage</small><b>${row.voltageR.toFixed(1)} V</b></span></td><td class="phase-cell" data-label="Phase S"><span><small>RMS Amp</small><strong>${row.currentS.toFixed(1)} A</strong></span><span><small>Voltage</small><b>${row.voltageS.toFixed(1)} V</b></span></td><td class="phase-cell" data-label="Phase T"><span><small>RMS Amp</small><strong>${row.currentT.toFixed(1)} A</strong></span><span><small>Voltage</small><b>${row.voltageT.toFixed(1)} V</b></span></td><td class="phase-cell" data-label="Load"><span><small>Avg Current</small><strong>${row.currentAverage.toFixed(1)} A</strong></span><span><small>Line Voltage</small><b>${row.lineVoltage.toFixed(0)} V</b></span></td><td class="phase-cell" data-label="Drive"><span><small>Active power</small><strong>${row.kw.toFixed(1)} kW</strong></span><span><small>Frequency</small><b>${row.hz.toFixed(1)} Hz</b></span></td><td class="phase-cell quality-cell" data-label="Quality"><span><small>Imbalance</small><strong>${row.imbalance.toFixed(1)}%</strong></span><span class="data-pill ${row.status === "Review" ? "warning" : "good"}">${row.status}</span></td></tr>`).join("")}</tbody></table></div>
