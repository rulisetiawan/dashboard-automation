...
const validProcesses = new Set(["calator", "dryer", "kalender", "chemical"]);
...
function assetRow(row: Record<string, any>) {
  return { id: row.asset_id, name: row.display_name, process: row.process_type, area: row.area_code, areaLabel: row.area_name, subtype: row.subtype, ...(row.config_json || {}), state: row.machine_state, batch: row.batch_no || "—", progress: Number(row.progress_percent), connected: row.connected, sourceTs: row.source_ts, quality: row.quality };
}
...
export class ApiController {
  ...
  async status() {
    ...
    return { data_mode: "SIMULATED_SEED", storage: "POSTGRESQL_NATIVE_LOCAL", framework: "NESTJS", scope: "NON_JETFLOW", processes: result.rows, gateway_ingestion: false, server_time: new Date().toISOString() };
  }

  @Get("assets")
  async assets(@Query("process") process?: string, @Query("area") area?: string) {
    if (!process || !validProcesses.has(process)) throw new BadRequestException("process must be calator, dryer, kalender, or chemical");
    const result = area
    ...
    return { data_mode: "SIMULATED_SEED", assets: result.rows.map(assetRow) };
  }
  ...
  async snapshot(@Param("assetId") assetId: string) {
    ...
    return { data_mode: "SIMULATED_SEED", asset: assetRow(asset.rows[0]), tags: tags.rows };
  }
